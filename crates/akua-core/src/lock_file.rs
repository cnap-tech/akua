...
impl AkuaLock {
    ...
    /// Write this lock to `<workspace>/akua.lock`. Canonicalizes order
    /// via [`sort`](Self::sort) before serializing so two calls with
    /// the same content produce byte-identical output.
    pub fn save(&self, workspace: &std::path::Path) -> Result<(), LockLoadError> {
        let mut lock = self.clone();
        lock.sort();
        let body = lock.to_toml().map_err(|source| LockLoadError::Parse {
            path: workspace.join("akua.lock"),
            source,
        })?;
        let path = workspace.join("akua.lock");
        let header = "# akua.lock — machine-maintained. Never hand-edit.\n\
                      # Regenerated by `akua add`, `akua pull`, `akua publish`, `akua update`.\n\n";
        let full = format!("{header}{body}");
        std::fs::write(&path, full).map_err(|source| LockLoadError::Io { path, source })?;
        Ok(())
    }

    /// Find the locked entry matching `name`. Returns the first match,
    /// which is the only one when deps have a single version; callers
    /// with multi-version scenarios should iterate `packages` directly.
