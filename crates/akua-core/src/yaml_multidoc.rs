//! Shared multi-document YAML parsing for shell-out engine callables.
//!
//! `helm template` + `kustomize build` (and any future WASM-embedded
//! successors) stream a single `---`-separated YAML document per
//! resource. Every handler wants the same thing: a `Vec<Value>`, one
//! entry per non-empty doc. Lifted here so each plugin doesn't carry
//! its own copy.

use serde_json::Value;

/// Parse a multi-document YAML byte slice into one `Value` per doc.
/// Empty separator docs (between resources) are dropped so the caller
/// can splat the result directly into `resources`.
///
/// `plugin_name` prefixes any error string so a failure inside
/// `helm.template` looks different from one inside `kustomize.build`
/// when it surfaces in a user's render output.
