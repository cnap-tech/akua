...
- **Bugs in the dep supply chain.** If a chart OCI registry is compromised and serves a malicious chart under a legitimate digest, akua renders it. Phase 6 (cosign verification) mitigates by requiring valid signatures; absent that, akua trusts the lockfile digests.
...
- **Path-traversal guard** on every plugin callable's path argument. `kcl_plugin::resolve_against_package` canonicalizes + asserts-under-package-dir + resolves symlinks. A Package that passes `"../../etc/passwd"` to `pkg.render(...)` gets a typed error, not a render.
...
|---|---|---|
| Path-traversal rejection in `resolve_against_package` | Not enforced — `../` paths resolve as-is | **Phase 0** |
| `helm.template` / `kustomize.build` via WASM engines | Shell-out, being removed | Phases 1 + 3 |
...
| cosign verification on OCI deps | Not implemented | Phase 6 |
